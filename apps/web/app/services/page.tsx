...
/* ══ data ══ */
const SERVICE_TYPES = [
  { key:'all',         label:'همه خدمات',       icon:'🛠️', color:'#10b981', count:48  },
  { key:'install',     label:'نصب میز',          icon:'🔧', color:'#06b6d4', count:18  },
  { key:'cloth',       label:'تعویض کلاث',       icon:'🟢', color:'#10b981', count:24  },
...
const PROVIDERS: Provider[] = [
  ...
  },
  {
    ...
    avatar:'ا', avatarColor:'#10b981', coverImg:'/images/billiadr-club-3.jpg',
    isVerified:true, isCertified:true, isOnline:true,
  ...
  return (
    <Link href={`/services/${p.id}`} style={{ textDecoration:'none', display:'block', height:'100%' }}>
      <div onMouseEnter={()=>setHov(true)} onMouseLeave={()=>setHov(false)}
        style={{ background: hov?'rgba(255,255,255,0.055)':'rgba(255,255,255,0.028)', border:`1px solid ${hov?`${p.avatarColor}28`:'rgba(255,255,255,0.07)'}`, borderRadius:'22px', overflow:'hidden', transition:'all 0.4s cubic-bezier(0.4,0,0.2,1)', transform: hov?'translateY(-7px)':'none', boxShadow: hov?`0 24px 60px rgba(0,0,0,0.5),0 0 0 1px ${p.avatarColor}08`:'0 4px 20px rgba(0,0,0,0.25)', height:'100%', display:'flex', flexDirection:'column' }}>
        ...
        <div style={{ height:'160px', position:'relative', overflow:'hidden', flexShrink:0 }}>
          ...
          <div style={{ position:'absolute', top:'12px', right:'12px', display:'flex', flexDirection:'column', gap:'5px' }}>
            ...
            {p.isOnline && <div style={{ display:'flex', alignItems:'center', gap:'3px', background:'rgba(16,185,129,0.15)', backdropFilter:'blur(6px)', border:'1px solid rgba(16,185,129,0.3)', borderRadius:'20px', padding:'3px 9px', fontSize:'9px', color:'#10b981', fontWeight:700 }}><span style={{ width:'5px', height:'5px', borderRadius:'50%', background:'#10b981', display:'inline-block', animation:'pulse 2s infinite' }}/>آنلاین</div>}
          </div>
        ...
        <div style={{ padding:'0 18px', marginTop:'-22px', display:'flex', alignItems:'flex-end', justifyContent:'space-between' }}>
          ...
          <div style={{ display:'flex', alignItems:'center', gap:'4px', background:'rgba(245,158,11,0.1)', border:'1px solid rgba(245,158,11,0.2)', borderRadius:'20px', padding:'4px 10px', marginBottom:'4px' }}>
            ...
            <span style={{ fontSize:'10px', color:'rgba(240,250,245,0.3)' }}>({toFa(p.reviewCount)})</span>
          </div>
        </div>
        ...
        <div style={{ padding:'10px 18px 0', flex:1, display:'flex', flexDirection:'column' }}>
          <div style={{ display:'flex', alignItems:'center', gap:'5px', marginBottom:'3px' }}>
            <h3 style={{ fontSize:'15px', fontWeight:900, color:'#f0faf5', margin:0, letterSpacing:'-0.015em' }}>{p.name}</h3>
            {p.isVerified && <Check size={12} style={{ color:p.avatarColor, flexShrink:0 }}/>}
          </div>
          <div style={{ fontSize:'11px', color:'rgba(240,250,245,0.45)', marginBottom:'10px' }}>{p.title}</div>
          ...
          {/* Quick info */}
          <div style={{ display:'flex', gap:'12px', marginBottom:'12px', fontSize:'11px', color:'rgba(240,250,245,0.4)' }}>
            <span style={{ display:'flex', alignItems:'center', gap:'3px' }}><Clock size={10} style={{ color:'#10b981' }}/>{p.responseTime}</span>
            <span style={{ display:'flex', alignItems:'center', gap:'3px' }}><Shield size={10} style={{ color:'#06b6d4' }}/>{p.warranty}</span>
          </div>
        </div>

        {/* Footer */}
        <div style={{ padding:'12px 18px 16px', marginTop:'auto', borderTop:'1px solid rgba(255,255,255,0.05)' }}>
          <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between' }}>
            <div>
              <div style={{ fontSize:'9px', color:'rgba(240,250,245,0.3)', marginBottom:'2px' }}>از</div>
            ...
            <div style={{ display:'flex', alignItems:'center', gap:'5px', fontSize:'11px', color:'rgba(240,250,245,0.35)' }}>
              <Wrench size={11} style={{ color:'rgba(240,250,245,0.2)' }}/>{toFa(p.jobs)} پروژه
            </div>
          </div>
        </div>
      </div>
      ...
      <div style={{ minHeight:'100vh', background:'linear-gradient(180deg,#020806,#060d0a)', paddingBottom:'80px' }}>
          ...
          <div style={{ maxWidth:'1280px', margin:'0 auto' }}>
            <div style={{ display:'flex', alignItems:'center', gap:'14px', marginBottom:'16px' }}>
              ...
              <div>
                ...
                <h1 style={{ fontSize:'clamp(24px,4.5vw,44px)', fontWeight:900, color:'#f0faf5', margin:0, letterSpacing:'-0.03em', lineHeight:1.1 }}>خدمات فنی تخصصی</h1>
              </div>
            </div>
            ...
            <div className="svc-types" style={{ display:'grid', gridTemplateColumns:'repeat(6,1fr)', gap:'10px', paddingBottom:'24px' }}>
              {SERVICE_TYPES.map(s => (
                <button key={s.key} onClick={()=>setService(s.key)}
                  style={{ padding:'14px 10px', borderRadius:'16px', background: service===s.key?`${s.color}10`:'rgba(255,255,255,0.025)', border:`1px solid ${service===s.key?`${s.color}35`:'rgba(255,255,255,0.07)'}`, cursor:'pointer', fontFamily:'inherit', textAlign:'center', transition:'all 0.3s', boxShadow: service===s.key?`0 4px 16px ${s.color}15`:'none' }}>
                  <div style={{ fontSize:'22px', marginBottom:'6px' }}>{s.icon}</div>
                  <div style={{ fontSize:'10px', fontWeight:700, color: service===s.key?s.color:'rgba(240,250,245,0.5)', lineHeight:1.3 }}>{s.label}</div>
                  <div style={{ fontSize:'9px', color:'rgba(240,250,245,0.25)', marginTop:'3px' }}>{toFa(s.count)} متخصص</div>
                </button>
              ))}
            </div>
          </div>
        ...
        <div style={{ background:'rgba(2,8,6,0.97)', borderBottom:'1px solid rgba(255,255,255,0.05)', padding:'12px clamp(16px,4vw,48px)', position:'sticky', top:'62px', zIndex:90, backdropFilter:'blur(24px)' }}>
          <div style={{ maxWidth:'1280px', margin:'0 auto', display:'flex', gap:'10px', alignItems:'center', flexWrap:'wrap' }}>
            ...
            <div style={{ fontSize:'12px', color:'rgba(240,250,245,0.35)', whiteSpace:'nowrap' }}>{toFa(filtered.length)} متخصص</div>
          </div>
        </div>
        ...
        <div style={{ maxWidth:'1280px', margin:'0 auto', padding:'clamp(24px,4vw,40px) clamp(16px,3vw,32px)' }}>
          ...
          <div style={{ display:'flex', gap:'32px', marginBottom:'36px', flexWrap:'wrap' }}>
            {[
              ...
              { v:toFa(PROVIDERS.filter(p=>p.isCertified).length),     l:'دارای گواهی',  c:'#10b981' },
          ...
          {/* How it works */}
          <div style={{ background:'rgba(255,255,255,0.02)', border:'1px solid rgba(255,255,255,0.06)', borderRadius:'22px', padding:'clamp(20px,3vw,30px)', marginBottom:'36px', overflow:'hidden', position:'relative' }}>
            ...
            <div style={{ display:'grid', gridTemplateColumns:'repeat(4,1fr)', gap:'20px' }}>
              {[
                ...
                <div key={i} style={{ textAlign:'center' }}>
                  ...
                  <div style={{ fontSize:'13px', fontWeight:700, color:'#f0faf5', marginBottom:'5px' }}>{s.title}</div>
                  <div style={{ fontSize:'11px', color:'rgba(240,250,245,0.4)', lineHeight:1.6 }}>{s.desc}</div>
                </div>
              ))}
            </div>
          </div>
          ...
          {filtered.length===0 ? (
            <div style={{ textAlign:'center', padding:'80px 24px' }}>
              ...
              <h3 style={{ fontSize:'18px', fontWeight:800, color:'#f0faf5', margin:'0 0 8px' }}>متخصصی یافت نشد</h3>
          ...
          <div style={{ marginTop:'56px', padding:'40px 36px', background:'rgba(245,158,11,0.03)', border:'1px dashed rgba(245,158,11,0.18)', borderRadius:'24px', textAlign:'center' }}>
            ...
            <h3 style={{ fontSize:'clamp(18px,3vw,26px)', fontWeight:900, color:'#f0faf5', margin:'0 0 10px', letterSpacing:'-0.025em' }}>متخصص شوید</h3>
