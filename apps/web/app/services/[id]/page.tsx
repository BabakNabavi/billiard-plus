...
const PROVIDERS: Record<string,any> = {
  s1: {
    ...
    avatar:'م', avatarColor:'#10b981', coverImg:'/images/billiadr-club-1.jpg',
  ...
  return (
    <>
      <style>{`
        ...
        .svc-row { display:flex; align-items:center; gap:16px; padding:16px 18px; background:rgba(255,255,255,0.025); border:1px solid rgba(255,255,255,0.07); border-radius:16px; cursor:pointer; transition:all 0.3s; }
        .svc-row:hover { background:rgba(255,255,255,0.045); border-color:rgba(16,185,129,0.2); }
      ...
      <div style={{ minHeight:'100vh', background:'linear-gradient(180deg,#020806,#060d0a)', paddingBottom:'80px' }}>
        ...
        <div style={{ position:'relative', height:'clamp(420px,55vh,580px)', overflow:'hidden' }}>
          ...
          <div style={{ position:'absolute', top:'24px', left:0, right:0, padding:'0 clamp(16px,4vw,48px)', display:'flex', justifyContent:'space-between', zIndex:10 }}>
            <Link href="/services" style={{ display:'flex', alignItems:'center', gap:'6px', color:'rgba(255,255,255,0.5)', fontSize:'12px', textDecoration:'none', background:'rgba(0,0,0,0.4)', backdropFilter:'blur(16px)', border:'1px solid rgba(255,255,255,0.08)', borderRadius:'10px', padding:'7px 14px' }}>
          ...
          <div style={{ position:'absolute', bottom:0, left:0, right:0, padding:'clamp(24px,4vw,52px)', zIndex:10, opacity:heroOp }}>
            <div style={{ display:'flex', alignItems:'flex-end', gap:'24px', flexWrap:'wrap' }}>
              ...
              <div style={{ position:'relative', flexShrink:0 }}>
                <div style={{ width:'clamp(72px,12vw,108px)', height:'clamp(72px,12vw,108px)', borderRadius:'22px', background:`linear-gradient(135deg,${p.avatarColor},${p.avatarColor}80)`, display:'flex', alignItems:'center', justifyContent:'center', fontSize:'clamp(28px,6vw,44px)', fontWeight:900, color:'#fff', border:'3px solid rgba(16,185,129,0.4)', boxShadow:`0 0 40px ${p.avatarColor}30,0 20px 60px rgba(0,0,0,0.5)` }}>
                  {p.avatar}
                </div>
                {p.isOnline && <div style={{ position:'absolute', bottom:'4px', right:'4px', width:'14px', height:'14px', borderRadius:'50%', background:'#10b981', border:'2px solid rgba(2,8,6,0.98)', boxShadow:'0 0 10px #10b981' }}/>}
              </div>
              ...
              <div style={{ flex:1, minWidth:'200px' }}>
                ...
                <div style={{ display:'flex', gap:'10px', flexWrap:'wrap' }}>
                  <div style={{ display:'flex', alignItems:'center', gap:'5px', background:'rgba(255,255,255,0.08)', backdropFilter:'blur(12px)', borderRadius:'20px', padding:'5px 13px', fontSize:'12px', color:'rgba(255,255,255,0.8)' }}>
                    <MapPin size={10} style={{ color:p.avatarColor }}/>{p.city}
                  </div>
                  <div style={{ display:'flex', gap:'3px', alignItems:'center', background:'rgba(255,255,255,0.08)', backdropFilter:'blur(12px)', borderRadius:'20px', padding:'5px 13px' }}>
                  ...
                  <div style={{ display:'flex', alignItems:'center', gap:'5px', background:'rgba(255,255,255,0.08)', backdropFilter:'blur(12px)', borderRadius:'20px', padding:'5px 13px', fontSize:'12px', color:'rgba(255,255,255,0.8)' }}>
                    <Clock size={10} style={{ color:'#10b981' }}/>{p.responseTime}
                  </div>
                </div>
              </div>
              ...
              <div style={{ display:'flex', gap:'10px', flexShrink:0, flexWrap:'wrap' }}>
                ...
                <button style={{ display:'flex', alignItems:'center', gap:'7px', padding:'12px 18px', borderRadius:'12px', background:'rgba(255,255,255,0.06)', border:'1px solid rgba(255,255,255,0.1)', color:'rgba(255,255,255,0.7)', fontSize:'13px', fontWeight:600, cursor:'pointer', fontFamily:'inherit' }}>
        ...
        <div style={{ maxWidth:'1280px', margin:'0 auto', padding:'clamp(24px,4vw,40px) clamp(16px,3vw,32px)' }}>
          ...
          <div className="prov-g" style={{ display:'grid', gridTemplateColumns:'1fr 320px', gap:'28px', alignItems:'start' }}>
            ...
            <div>
              ...
              {tab==='overview' && (
                <div style={{ animation:'fadeUp 0.4s ease both', display:'flex', flexDirection:'column', gap:'20px' }}>
                  ...
                  <div style={{ background:'rgba(255,255,255,0.025)', border:'1px solid rgba(255,255,255,0.07)', borderRadius:'20px', padding:'26px' }}>
                    ...
                    <p style={{ fontSize:'14px', color:'rgba(240,250,245,0.55)', lineHeight:1.9, margin:'0 0 20px' }}>{p.bio}</p>
                    ...
                    <div style={{ display:'grid', gridTemplateColumns:'repeat(2,1fr)', gap:'10px' }}>
                      {[
                        ...
                        { l:'ضمانت کار',   v:p.warranty,        icon:<Shield size={13} style={{ color:'#10b981' }}/> },
                        ...
                        <div key={i} style={{ display:'flex', alignItems:'center', gap:'10px', padding:'12px 14px', background:'rgba(255,255,255,0.02)', border:'1px solid rgba(255,255,255,0.05)', borderRadius:'12px' }}>
                          ...
                          <div style={{ minWidth:0 }}>
                            ...
                            <div style={{ fontSize:'13px', fontWeight:600, color:'#f0faf5', overflow:'hidden', textOverflow:'ellipsis', whiteSpace:'nowrap' }}>{r.v}</div>
                          </div>
                        </div>
                      ))}
                    </div>
                  ...
                  {p.beforeAfter.length > 0 && (
                    <div style={{ background:'rgba(255,255,255,0.025)', border:'1px solid rgba(255,255,255,0.07)', borderRadius:'20px', overflow:'hidden' }}>
                      <div style={{ padding:'22px 24px 16px', display:'flex', justifyContent:'space-between', alignItems:'center' }}>
                        <h3 style={{ fontSize:'15px', fontWeight:800, color:'#f0faf5', margin:0, display:'flex', alignItems:'center', gap:'10px' }}>
                        ...
                        <div style={{ display:'flex', gap:'6px' }}>
                          <button onClick={()=>setBaIdx(i=>Math.max(0,i-1))} style={{ width:'28px', height:'28px', borderRadius:'8px', background:'rgba(255,255,255,0.04)', border:'1px solid rgba(255,255,255,0.08)', cursor:'pointer', color:'rgba(240,250,245,0.5)', display:'flex', alignItems:'center', justifyContent:'center' }}><ChevronRight size={14}/></button>
                          <button onClick={()=>setBaIdx(i=>Math.min(p.beforeAfter.length-1,i+1))} style={{ width:'28px', height:'28px', borderRadius:'8px', background:'rgba(255,255,255,0.04)', border:'1px solid rgba(255,255,255,0.08)', cursor:'pointer', color:'rgba(240,250,245,0.5)', display:'flex', alignItems:'center', justifyContent:'center' }}><ChevronLeft size={14}/></button>
                        </div>
                      </div>
                      ...
                      {p.beforeAfter[baIdx] && (
                        <div style={{ padding:'0 24px 20px' }}>
                          ...
                          <div style={{ fontSize:'13px', color:'rgba(240,250,245,0.5)', textAlign:'center' }}>{p.beforeAfter[baIdx].title}</div>
                        </div>
                      )}
                    </div>
                  )}
                ...
                <div style={{ animation:'fadeUp 0.4s ease both', display:'flex', flexDirection:'column', gap:'10px' }}>
                  {p.services.map((svc: any, i: number) => (
                    <div key={i} className="svc-row" onClick={()=>setSelSvc(selSvc===i?null:i)}
                      style={{ borderColor: selSvc===i?`${p.avatarColor}40`:'rgba(255,255,255,0.07)', background: selSvc===i?`${p.avatarColor}08`:'rgba(255,255,255,0.025)' }}>
                      ...
                      <div style={{ flex:1, minWidth:0 }}>
                        <div style={{ display:'flex', alignItems:'center', gap:'8px', marginBottom:'4px' }}>
                          <span style={{ fontSize:'14px', fontWeight:700, color:'#f0faf5' }}>{svc.title}</span>
                        ...
                        <div style={{ display:'flex', gap:'10px', fontSize:'11px', color:'rgba(240,250,245,0.4)' }}>
                          <span style={{ display:'flex', alignItems:'center', gap:'3px' }}><Clock size={10} style={{ color:'rgba(240,250,245,0.25)' }}/>{svc.duration}</span>
                          <span style={{ display:'flex', alignItems:'center', gap:'3px' }}><Shield size={10} style={{ color:'rgba(240,250,245,0.25)' }}/>{p.warranty} ضمانت</span>
                        </div>
                      </div>
                      <div style={{ textAlign:'left', flexShrink:0 }}>
                        ...
                        <div style={{ fontSize:'10px', color:'rgba(240,250,245,0.3)' }}>تومان</div>
                      </div>
                  ...
                  {selSvc !== null && (
                    <div style={{ padding:'16px 18px', background:`${p.avatarColor}08`, border:`1px solid ${p.avatarColor}22`, borderRadius:'16px', display:'flex', alignItems:'center', justifyContent:'space-between', flexWrap:'wrap', gap:'12px', animation:'fadeUp 0.3s ease both' }}>
                      <div>
                        <div style={{ fontSize:'14px', fontWeight:800, color:'#f0faf5', marginBottom:'3px' }}>{p.services[selSvc]?.title}</div>
                        <div style={{ fontSize:'12px', color:'rgba(240,250,245,0.4)' }}>هزینه: {toFa(p.services[selSvc]?.price.toLocaleString())} تومان</div>
                      </div>
              ...
              {tab==='reviews' && (
                <div style={{ animation:'fadeUp 0.4s ease both', display:'flex', flexDirection:'column', gap:'12px' }}>
                  {/* Summary */}
                  <div style={{ background:'rgba(255,255,255,0.025)', border:'1px solid rgba(255,255,255,0.07)', borderRadius:'20px', padding:'24px', display:'flex', gap:'24px', alignItems:'center', flexWrap:'wrap' }}>
                    <div style={{ textAlign:'center', flexShrink:0 }}>
                      <div style={{ fontSize:'52px', fontWeight:900, color:'#f0faf5', lineHeight:1 }}>{p.rating}</div>
                      ...
                      <div style={{ fontSize:'11px', color:'rgba(240,250,245,0.35)' }}>{toFa(p.reviewCount)} نظر</div>
                    </div>
                    <div style={{ flex:1, minWidth:'160px', display:'flex', flexDirection:'column', gap:'6px' }}>
                      {[{s:5,pct:88},{s:4,pct:9},{s:3,pct:2},{s:2,pct:1},{s:1,pct:0}].map(r => (
                        <div key={r.s} style={{ display:'flex', alignItems:'center', gap:'8px' }}>
                          <span style={{ fontSize:'10px', color:'rgba(240,250,245,0.35)', width:'12px' }}>{toFa(r.s)}</span>
                          ...
                          <span style={{ fontSize:'10px', color:'rgba(240,250,245,0.25)', width:'26px', textAlign:'left' }}>{toFa(r.pct)}٪</span>
                        </div>
                      ))}
                    </div>
                  </div>
                  ...
                  {REVIEWS.map((r,i) => (
                    <div key={i} style={{ padding:'20px', background:'rgba(255,255,255,0.025)', border:'1px solid rgba(255,255,255,0.06)', borderRadius:'18px' }}>
                        ...
                        <div style={{ display:'flex', alignItems:'center', gap:'12px' }}>
                          ...
                          <div>
                            <div style={{ display:'flex', alignItems:'center', gap:'7px', marginBottom:'3px' }}>
                              ...
                              {r.verified && <span style={{ fontSize:'9px', color:'#10b981', background:'rgba(16,185,129,0.1)', border:'1px solid rgba(16,185,129,0.2)', borderRadius:'20px', padding:'1px 7px', fontWeight:700, display:'flex', alignItems:'center', gap:'2px' }}><Check size={8}/>تأیید</span>}
                      ...
                      <p style={{ fontSize:'13px', color:'rgba(240,250,245,0.55)', margin:'0 0 12px', lineHeight:1.75 }}>{r.text}</p>
                      <button style={{ display:'flex', alignItems:'center', gap:'5px', padding:'5px 12px', borderRadius:'20px', background:'rgba(255,255,255,0.04)', border:'1px solid rgba(255,255,255,0.07)', fontSize:'11px', color:'rgba(240,250,245,0.45)', cursor:'pointer', fontFamily:'inherit', transition:'all 0.2s' }}>
            ...
            <div style={{ position:'sticky', top:'80px', display:'flex', flexDirection:'column', gap:'16px' }}>
              ...
              {/* Book CTA */}
              <div style={{ background:'rgba(255,255,255,0.025)', border:`1px solid ${p.avatarColor}22`, borderRadius:'22px', padding:'22px', position:'relative', overflow:'hidden' }}>
                ...
                <div style={{ fontSize:'13px', color:'rgba(240,250,245,0.5)', marginBottom:'16px', textAlign:'center', lineHeight:1.6 }}>
                  از {toFa(p.priceFrom.toLocaleString())} تومان
                </div>

                {/* Quick info */}
                {[
                  { icon:<Clock size={13}/>,  label:'پاسخ',  v:p.responseTime, c:'#10b981' },
                  { icon:<Shield size={13}/>, label:'ضمانت', v:p.warranty,     c:'#06b6d4' },
                ].map((x,i) => (
                  <div key={i} style={{ display:'flex', alignItems:'center', gap:'10px', padding:'9px 12px', background:'rgba(255,255,255,0.02)', border:'1px solid rgba(255,255,255,0.05)', borderRadius:'11px', marginBottom:'8px' }}>
                    <span style={{ color:x.c, flexShrink:0 }}>{x.icon}</span>
                    <span style={{ fontSize:'12px', color:'rgba(240,250,245,0.4)' }}>{x.label}:</span>
                    <span style={{ fontSize:'12px', fontWeight:600, color:'#f0faf5', marginRight:'auto' }}>{x.v}</span>
                  </div>
                ))}

                <button onClick={()=>setReq(true)} style={{ width:'100%', padding:'14px', marginTop:'8px', borderRadius:'13px', border:'none', background: requested?'rgba(16,185,129,0.2)':`linear-gradient(135deg,${p.avatarColor},${p.avatarColor}cc)`, color: requested?'#10b981':'#fff', fontSize:'14px', fontWeight:800, cursor:'pointer', fontFamily:'inherit', transition:'all 0.3s', boxShadow: requested?'none':`0 8px 24px ${p.avatarColor}30`, display:'flex', alignItems:'center', justifyContent:'center', gap:'9px' }}>
              ...
              <div style={{ background:'rgba(255,255,255,0.025)', border:'1px solid rgba(255,255,255,0.07)', borderRadius:'20px', padding:'20px' }}>
                <div style={{ fontSize:'11px', color:'rgba(240,250,245,0.3)', letterSpacing:'0.15em', fontWeight:700, marginBottom:'12px', textTransform:'uppercase' }}>تخصص‌ها</div>
              ...
              {/* Stats */}
              <div style={{ background:'rgba(255,255,255,0.025)', border:'1px solid rgba(255,255,255,0.07)', borderRadius:'20px', padding:'20px' }}>
                <div style={{ fontSize:'11px', color:'rgba(240,250,245,0.3)', letterSpacing:'0.15em', fontWeight:700, marginBottom:'14px', textTransform:'uppercase' }}>آمار کاری</div>
                {[
                  ...
                  { l:'گواهینامه',        v:toFa(p.certifications.length), c:'#10b981' },
                ].map((s,i) => (
