...
function LiveControl({ match }: { match: LiveMatch }) {
    ...
    const accentMap: Record<string, string> = { live: '#ef4444', break: '#f59e0b', completed: '#10b981', scheduled: '#06b6d4', disputed: '#a78bfa' };
    const accent = accentMap[status] ?? '#10b981';
    ...
    return (
        <div style={{ background: 'rgba(255,255,255,0.025)', border: `1px solid ${accent}20`, borderRadius: '22px', overflow: 'hidden', position: 'relative' }}>
            {/* Glow top */}
            ...
            {/* Header */}
            <div style={{ padding: '16px 20px', borderBottom: '1px solid rgba(255,255,255,0.05)', display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: '10px' }}>
                <div>
                    <div style={{ fontSize: '10px', color: 'rgba(240,250,245,0.35)', letterSpacing: '0.12em', marginBottom: '3px' }}>{match.tournament} · {match.round}</div>
                    <div style={{ fontSize: '14px', fontWeight: 800, color: '#f0faf5', display: 'flex', alignItems: 'center', gap: '8px' }}>
                        {match.table}
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: '6px', fontSize: '14px', fontWeight: 800, color: 'rgba(240,250,245,0.5)', fontVariantNumeric: 'tabular-nums' }}>
            ...
            <div style={{ padding: '20px', display: 'grid', gridTemplateColumns: '1fr auto 1fr', gap: '16px', alignItems: 'center' }}>
                ...
                <div style={{ textAlign: 'right' }}>
                    <div style={{ fontSize: 'clamp(13px,2vw,15px)', fontWeight: 800, color: '#f0faf5', marginBottom: '8px', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                        {match.player1.split(' ').pop()}
                    </div>
                    {/* Score buttons */}
                    <div style={{ display: 'flex', alignItems: 'center', gap: '8px', justifyContent: 'flex-end' }}>
                        <button onClick={() => setScore1(s => Math.max(0, s - 1))} style={{ width: '28px', height: '28px', borderRadius: '8px', background: 'rgba(255,255,255,0.04)', border: '1px solid rgba(255,255,255,0.08)', cursor: 'pointer', color: 'rgba(240,250,245,0.5)', display: 'flex', alignItems: 'center', justifyContent: 'center', transition: 'all 0.2s' }}>
                            <ChevronDown size={13} />
                        </button>
                        <div style={{ fontSize: '42px', fontWeight: 900, color: score1 > score2 ? '#10b981' : '#f0faf5', letterSpacing: '-0.04em', lineHeight: 1, textShadow: score1 > score2 ? '0 0 24px rgba(16,185,129,0.5)' : 'none', transition: 'all 0.3s' }}>
                            {toFa(score1)}
                        </div>
                        <button onClick={() => setScore1(s => s + 1)} style={{ width: '28px', height: '28px', borderRadius: '8px', background: 'rgba(16,185,129,0.1)', border: '1px solid rgba(16,185,129,0.25)', cursor: 'pointer', color: '#10b981', display: 'flex', alignItems: 'center', justifyContent: 'center', transition: 'all 0.2s' }}>
                            <ChevronUp size={13} />
                        </button>
                    </div>
                    <div style={{ fontSize: '11px', color: 'rgba(240,250,245,0.35)', marginTop: '6px' }}>بالاترین بریک: {toFa(match.p1Break)}</div>
                </div>

                {/* Center */}
                <div style={{ textAlign: 'center' }}>
                    <div style={{ fontSize: '12px', color: 'rgba(240,250,245,0.25)', marginBottom: '4px' }}>فریم {toFa(score1 + score2)} از {toFa(match.totalFrames)}</div>
                    <div style={{ fontSize: '16px', fontWeight: 900, color: 'rgba(240,250,245,0.2)', letterSpacing: '-0.02em' }}>:</div>
                    <div style={{ marginTop: '8px' }}>
                        <div style={{ fontSize: '11px', color: curBreak > 0 ? '#f59e0b' : 'rgba(240,250,245,0.2)', fontWeight: 700 }}>
                ...
                <div style={{ textAlign: 'left' }}>
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                        <button onClick={() => setScore2(s => Math.max(0, s - 1))} style={{ width: '28px', height: '28px', borderRadius: '8px', background: 'rgba(255,255,255,0.04)', border: '1px solid rgba(255,255,255,0.08)', cursor: 'pointer', color: 'rgba(240,250,245,0.5)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                            <ChevronDown size={13} />
                        </button>
                        <div style={{ fontSize: '42px', fontWeight: 900, color: score2 > score1 ? '#10b981' : '#f0faf5', letterSpacing: '-0.04em', lineHeight: 1, textShadow: score2 > score1 ? '0 0 24px rgba(16,185,129,0.5)' : 'none', transition: 'all 0.3s' }}>
                            {toFa(score2)}
                        </div>
                        <button onClick={() => setScore2(s => s + 1)} style={{ width: '28px', height: '28px', borderRadius: '8px', background: 'rgba(16,185,129,0.1)', border: '1px solid rgba(16,185,129,0.25)', cursor: 'pointer', color: '#10b981', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                            <ChevronUp size={13} />
                        </button>
                    </div>
                    <div style={{ fontSize: '11px', color: 'rgba(240,250,245,0.35)', marginTop: '6px' }}>بالاترین بریک: {toFa(match.p2Break)}</div>
                </div>
            </div>
            ...
            <div style={{ padding: '0 20px 16px' }}>
                <div style={{ display: 'flex', gap: '3px' }}>
                    {Array.from({ length: match.totalFrames }).map((_, i) => (
                        <div key={i} style={{ flex: 1, height: '5px', borderRadius: '3px', background: i < score1 ? '#10b981' : i >= match.totalFrames - score2 ? '#06b6d4' : 'rgba(255,255,255,0.07)', boxShadow: i < score1 ? '0 0 6px rgba(16,185,129,0.5)' : i >= match.totalFrames - score2 ? '0 0 6px rgba(6,182,212,0.5)' : 'none', transition: 'all 0.3s' }} />
                    ))}
                </div>
            </div>

            {/* Controls */}
            <div style={{ padding: '14px 20px 18px', borderTop: '1px solid rgba(255,255,255,0.05)', display: 'flex', gap: '8px', flexWrap: 'wrap', alignItems: 'center' }}>
                {/* Match controls */}
                <div style={{ display: 'flex', gap: '6px', flex: 1, flexWrap: 'wrap' }}>
                    {[
                        { icon: <Play size={13} />, label: 'ادامه', color: '#10b981', action: () => setStatus('live'), show: status === 'break' || status === 'scheduled' },
            ...
            <div style={{ padding: '0 20px 16px', display: 'flex', gap: '8px', alignItems: 'center', flexWrap: 'wrap' }}>
                <div style={{ fontSize: '11px', color: 'rgba(240,250,245,0.35)' }}>بریک جاری:</div>
                ...
                <div style={{ position: 'fixed', inset: 0, zIndex: 9999, background: 'rgba(0,0,0,0.8)', backdropFilter: 'blur(16px)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                    <div style={{ background: 'rgba(6,13,10,0.98)', border: '1px solid rgba(167,139,250,0.2)', borderRadius: '22px', padding: '28px', width: 'min(420px,92vw)', boxShadow: '0 40px 100px rgba(0,0,0,0.6)' }}>
                        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '20px' }}>
                            <h3 style={{ fontSize: '16px', fontWeight: 800, color: '#f0faf5', margin: 0 }}>ثبت اعتراض</h3>
                            <button onClick={() => setFoulModal(false)} style={{ background: 'rgba(255,255,255,0.06)', border: '1px solid rgba(255,255,255,0.1)', borderRadius: '50%', width: '32px', height: '32px', cursor: 'pointer', color: 'rgba(240,250,245,0.6)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                                <X size={14} />
                            </button>
                        </div>
                        <div style={{ display: 'flex', flexDirection: 'column', gap: '10px', marginBottom: '18px' }}>
                            {['خطای تکنیکی', 'خروج توپ', 'مشکل تجهیزات', 'رفتار نامناسب', 'سایر'].map(opt => (
                                <button key={opt} onClick={() => setFoulModal(false)} style={{ padding: '12px 16px', background: 'rgba(167,139,250,0.06)', border: '1px solid rgba(167,139,250,0.15)', borderRadius: '12px', color: 'rgba(240,250,245,0.7)', fontSize: '13px', fontWeight: 600, cursor: 'pointer', fontFamily: 'inherit', textAlign: 'right', transition: 'all 0.2s' }}
                                    onMouseEnter={e => { (e.currentTarget).style.background = 'rgba(167,139,250,0.12)'; (e.currentTarget).style.color = '#a78bfa'; }}
                                    onMouseLeave={e => { (e.currentTarget).style.background = 'rgba(167,139,250,0.06)'; (e.currentTarget).style.color = 'rgba(240,250,245,0.7)'; }}>
                                    {opt}
                                </button>
                            ))}
                        </div>
...
function DashboardContent() {
    ...
    return (
        ...
        .r-tab.active { background:rgba(16,185,129,0.1); border-color:rgba(16,185,129,0.3); color:#10b981; }
        .r-tab:not(.active) { background:rgba(255,255,255,0.03); color:rgba(240,250,245,0.45); }
        .r-tab:not(.active):hover { background:rgba(255,255,255,0.06); color:rgba(240,250,245,0.75); }

        .r-card { background:rgba(255,255,255,0.025); border:1px solid rgba(255,255,255,0.07); border-radius:20px; padding:22px; transition:all 0.3s; }
        .r-card:hover { background:rgba(255,255,255,0.04); }

        .sch-row { display:flex; align-items:center; gap:14px; padding:14px 16px; background:rgba(255,255,255,0.02); border:1px solid rgba(255,255,255,0.05); border-radius:14px; transition:all 0.25s; }
        .sch-row:hover { background:rgba(255,255,255,0.04); border-color:rgba(255,255,255,0.09); }
      ...
      `}</style>
            ...
            <div style={{ minHeight: '100vh', background: 'linear-gradient(180deg,#020806,#060d0a)', paddingBottom: '80px' }}>
                ...
                <div style={{ background: 'rgba(2,8,6,0.98)', borderBottom: '1px solid rgba(255,255,255,0.05)', padding: '0 clamp(16px,4vw,40px)', position: 'sticky', top: '62px', zIndex: 90, backdropFilter: 'blur(24px)' }}>
                    <div style={{ maxWidth: '1280px', margin: '0 auto', height: '58px', display: 'flex', alignItems: 'center', gap: '16px' }}>
                        <div style={{ display: 'flex', alignItems: 'center', gap: '12px', flex: 1, minWidth: 0 }}>
                            <div style={{ width: '38px', height: '38px', borderRadius: '11px', background: 'linear-gradient(135deg,#10b981,#059669)', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0, boxShadow: '0 4px 12px rgba(16,185,129,0.3)' }}>
                                <Shield size={18} style={{ color: '#fff' }} />
                            </div>
                            <div>
                                <div style={{ fontSize: '9px', color: 'rgba(16,185,129,0.6)', letterSpacing: '0.2em', fontWeight: 700 }}>REFEREE DASHBOARD</div>
                                <div style={{ fontSize: '15px', fontWeight: 800, color: '#f0faf5', letterSpacing: '-0.01em' }}>پنل داور رسمی</div>
                            </div>
                        </div>
                ...
                {/* ── Tab nav ── */}
                <div style={{ background: 'rgba(2,8,6,0.97)', borderBottom: '1px solid rgba(255,255,255,0.04)', padding: '0 clamp(16px,4vw,40px)', overflowX: 'auto' }}>
                    <div style={{ maxWidth: '1280px', margin: '0 auto', display: 'flex', gap: '4px', padding: '10px 0' }}>
                        {[
                            ...
                            <button key={t.k} className={`r-tab ${tab === t.k ? 'active' : ''}`} onClick={() => setTab(t.k as Tab)}>
                                {t.icon}{t.l}
                                {(t.badge ?? 0) > 0 && <span style={{ minWidth: '18px', height: '18px', borderRadius: '9px', background: tab === t.k ? '#10b981' : '#ef4444', color: '#fff', fontSize: '9px', fontWeight: 900, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '0 4px' }}>{toFa(t.badge!)}</span>}
                            </button>
                        ))}
                    </div>
                </div>
                ...
                <div style={{ maxWidth: '1280px', margin: '0 auto', padding: 'clamp(20px,3vw,36px) clamp(16px,3vw,32px)' }}>
                    ...
                    {tab === 'overview' && (
                        <div style={{ animation: 'fadeUp 0.4s ease both' }}>
                            ...
                            <div className="kpi-g" style={{ display: 'grid', gridTemplateColumns: 'repeat(4,1fr)', gap: '14px', marginBottom: '24px' }}>
                                {[
                                    { l: 'مسابقات امسال', v: toFa(284), sub: 'داوری‌شده', c: '#10b981', icon: <Target size={16} /> },
                                    ...
                                    <div key={i} className="r-card" style={{ animationDelay: `${i * 0.07}s`, animation: 'fadeUp 0.5s ease both' }}>
                                        ...
                                        <div style={{ fontSize: 'clamp(22px,3vw,28px)', fontWeight: 900, color: '#f0faf5', letterSpacing: '-0.03em', marginBottom: '4px', textShadow: `0 0 20px ${s.c}25` }}>{s.v}</div>
                                        <div style={{ fontSize: '12px', color: 'rgba(240,250,245,0.5)', fontWeight: 600, marginBottom: '2px' }}>{s.l}</div>
                            ...
                            <div className="ref-dash-g" style={{ display: 'grid', gridTemplateColumns: '1fr 320px', gap: '20px' }}>
                                ...
                                <div className="r-card">
                                    <div style={{ fontSize: '15px', fontWeight: 800, color: '#f0faf5', marginBottom: '16px', display: 'flex', alignItems: 'center', gap: '10px' }}>
                                        ...
                                        <button onClick={() => setTab('live')} style={{ marginRight: 'auto', fontSize: '11px', color: '#10b981', background: 'none', border: 'none', cursor: 'pointer', fontFamily: 'inherit' }}>مدیریت →</button>
                                    </div>
                                        ...
                                        {LIVE_MATCHES.map(m => (
                                            <div key={m.id} style={{ display: 'flex', alignItems: 'center', gap: '14px', padding: '14px 16px', background: m.status === 'live' ? 'rgba(239,68,68,0.04)' : 'rgba(245,158,11,0.04)', border: `1px solid ${m.status === 'live' ? 'rgba(239,68,68,0.15)' : 'rgba(245,158,11,0.15)'}`, borderRadius: '14px', cursor: 'pointer', transition: 'all 0.2s' }}
                                                ...
                                                <div style={{ flex: 1, minWidth: 0 }}>
                                                    <div style={{ fontSize: '13px', fontWeight: 700, color: '#f0faf5', marginBottom: '2px' }}>
                                                        {m.player1.split(' ').pop()} {toFa(m.score1)}:{toFa(m.score2)} {m.player2.split(' ').pop()}
                                                    </div>
                                                    <div style={{ fontSize: '11px', color: 'rgba(240,250,245,0.4)' }}>{m.round} · {m.table}</div>
                                                </div>
                                ...
                                <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
                                    ...
                                    {/* Profile card */}
                                    <div className="r-card" style={{ border: '1px solid rgba(16,185,129,0.2)', textAlign: 'center' }}>
                                        ...
                                        <div style={{ width: '56px', height: '56px', borderRadius: '16px', background: 'linear-gradient(135deg,#10b981,#059669)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '22px', fontWeight: 900, color: '#fff', margin: '0 auto 12px', boxShadow: '0 8px 24px rgba(16,185,129,0.3)' }}>
                                            {user?.firstName?.[0] ?? 'D'}
                                        </div>
                                        <div style={{ fontSize: '16px', fontWeight: 800, color: '#f0faf5', marginBottom: '4px' }}>{user?.firstName} {user?.lastName}</div>
                                        <div style={{ fontSize: '12px', color: 'rgba(240,250,245,0.4)', marginBottom: '12px' }}>داور بین‌المللی اسنوکر</div>
                                        <div style={{ display: 'flex', gap: '6px', justifyContent: 'center', flexWrap: 'wrap' }}>
                                            ...
                                            <span style={{ fontSize: '9px', color: '#10b981', background: 'rgba(16,185,129,0.1)', border: '1px solid rgba(16,185,129,0.2)', borderRadius: '20px', padding: '3px 10px', fontWeight: 700, display: 'flex', alignItems: 'center', gap: '3px' }}>
                                    ...
                                    <div className="r-card">
                                        ...
                                        {SCHEDULED.filter(m => m.date === 'امروز').map((m, i) => (
                                            <div key={i} style={{ display: 'flex', alignItems: 'center', gap: '10px', padding: '9px 0', borderBottom: i < SCHEDULED.filter(m => m.date === 'امروز').length - 1 ? '1px solid rgba(255,255,255,0.04)' : 'none' }}>
                                                <div style={{ fontSize: '12px', fontWeight: 800, color: '#10b981', flexShrink: 0, fontVariantNumeric: 'tabular-nums' }}>{m.time}</div>
                                                <div style={{ flex: 1, minWidth: 0 }}>
                                                    <div style={{ fontSize: '11px', color: '#f0faf5', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                                                        {m.player1.split(' ').pop()} vs {m.player2.split(' ').pop()}
                                                    </div>
                                                    <div style={{ fontSize: '9px', color: 'rgba(240,250,245,0.3)' }}>{m.table} · {m.round}</div>
                                                </div>
                                            </div>
                                        ))}
                                    </div>
                                    ...
                                    {DISPUTES.filter(d => d.status === 'open').length > 0 && (
                                        <div className="r-card" style={{ border: '1px solid rgba(239,68,68,0.15)' }}>
                                            <div style={{ fontSize: '13px', fontWeight: 700, color: '#f0faf5', marginBottom: '12px', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                                                <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                                            ...
                                            {DISPUTES.filter(d => d.status === 'open').map(d => (
                                                <div key={d.id} style={{ padding: '10px 12px', background: 'rgba(239,68,68,0.05)', border: '1px solid rgba(239,68,68,0.1)', borderRadius: '12px', cursor: 'pointer', marginBottom: '6px' }}
                                                    ...
                                                    <div style={{ fontSize: '12px', fontWeight: 600, color: '#f0faf5', marginBottom: '3px' }}>{d.issue}</div>
                                                    <div style={{ fontSize: '10px', color: 'rgba(240,250,245,0.4)' }}>{d.match} · {d.time}</div>
                                                </div>
                                            ))}
                        ...
                        <div style={{ animation: 'fadeUp 0.4s ease both' }}>
                            <div className="r-card" style={{ marginBottom: '16px' }}>
                                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '18px' }}>
                                    <div style={{ fontSize: '15px', fontWeight: 800, color: '#f0faf5', display: 'flex', alignItems: 'center', gap: '10px' }}>
                                    ...
                                    <div style={{ display: 'flex', gap: '6px' }}>
                                        {['امروز', 'فردا', 'این هفته'].map((d, i) => (
                                            <button key={i} style={{ padding: '6px 13px', borderRadius: '20px', background: i === 0 ? 'rgba(16,185,129,0.1)' : 'rgba(255,255,255,0.03)', border: `1px solid ${i === 0 ? 'rgba(16,185,129,0.3)' : 'rgba(255,255,255,0.07)'}`, color: i === 0 ? '#10b981' : 'rgba(240,250,245,0.45)', fontSize: '11px', fontWeight: 600, cursor: 'pointer', fontFamily: 'inherit' }}>
                                                {d}
                                            </button>
                                        ))}
                                    </div>
                                ...
                                <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
                                    {SCHEDULED.map(m => {
                                        const statusCfg = m.status === 'scheduled' ? { color: '#06b6d4', label: 'برنامه‌ریزی شده' } : { color: '#10b981', label: 'در حال بازی' };
                                        return (
                                            <div key={m.id} className="sch-row">
                                                <div style={{ textAlign: 'center', flexShrink: 0, minWidth: '52px' }}>
                                                    <div style={{ fontSize: '14px', fontWeight: 900, color: '#10b981', fontVariantNumeric: 'tabular-nums' }}>{m.time}</div>
                                                    <div style={{ fontSize: '10px', color: 'rgba(240,250,245,0.3)' }}>{m.date}</div>
                                                </div>
                                                <div style={{ flex: 1, minWidth: 0 }}>
                                                    <div style={{ fontSize: '13px', fontWeight: 700, color: '#f0faf5', marginBottom: '3px' }}>
                                                        {m.player1} vs {m.player2}
                                                    </div>
                                                    <div style={{ fontSize: '11px', color: 'rgba(240,250,245,0.4)', display: 'flex', gap: '8px', flexWrap: 'wrap' }}>
                        ...
                        <div style={{ animation: 'fadeUp 0.4s ease both' }}>
                            <div className="r-card">
                                <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '18px' }}>
                                    ...
                                    <button style={{ display: 'flex', alignItems: 'center', gap: '6px', padding: '9px 16px', background: 'linear-gradient(135deg,#10b981,#059669)', border: 'none', borderRadius: '10px', color: '#fff', fontSize: '12px', fontWeight: 700, cursor: 'pointer', fontFamily: 'inherit' }}>
                                    ...
                                    {REPORTS.map((r, i) => (
                                        <div key={i} className="sch-row" style={{ flexWrap: 'wrap', gap: '10px' }}>
                                            <div style={{ flex: 1, minWidth: '160px' }}>
                                                <div style={{ fontSize: '13px', fontWeight: 700, color: '#f0faf5', marginBottom: '3px' }}>{r.match}</div>
                                                <div style={{ fontSize: '11px', color: 'rgba(240,250,245,0.4)', display: 'flex', gap: '8px', flexWrap: 'wrap' }}>
                                            ...
                                            <div style={{ display: 'flex', gap: '6px', alignItems: 'center', flexShrink: 0 }}>
                                                <span style={{ fontSize: '10px', padding: '3px 10px', borderRadius: '20px', background: r.submitted ? 'rgba(16,185,129,0.1)' : 'rgba(245,158,11,0.1)', border: `1px solid ${r.submitted ? 'rgba(16,185,129,0.25)' : 'rgba(245,158,11,0.25)'}`, color: r.submitted ? '#10b981' : '#f59e0b', fontWeight: 700 }}>
                                                    {r.submitted ? 'ارسال شده' : 'در انتظار'}
                                                </span>
                                                <button style={{ padding: '6px 12px', background: 'rgba(255,255,255,0.04)', border: '1px solid rgba(255,255,255,0.08)', borderRadius: '9px', color: 'rgba(240,250,245,0.6)', fontSize: '11px', fontWeight: 600, cursor: 'pointer', fontFamily: 'inherit', display: 'flex', alignItems: 'center', gap: '5px' }}>
                    ...
                    {tab === 'disputes' && (
                        <div style={{ animation: 'fadeUp 0.4s ease both', display: 'flex', flexDirection: 'column', gap: '10px' }}>
                            {DISPUTES.map(d => (
                                <div key={d.id} style={{ padding: '18px 20px', background: d.status === 'open' ? 'rgba(239,68,68,0.04)' : d.status === 'resolved' ? 'rgba(16,185,129,0.03)' : 'rgba(255,255,255,0.025)', border: `1px solid ${d.status === 'open' ? 'rgba(239,68,68,0.18)' : d.status === 'resolved' ? 'rgba(16,185,129,0.15)' : 'rgba(255,255,255,0.07)'}`, borderRadius: '18px' }}>
                                    <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', gap: '12px', marginBottom: '10px', flexWrap: 'wrap' }}>
                                        <div>
                                            <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '4px' }}>
                                                <span style={{ fontSize: '15px', fontWeight: 800, color: '#f0faf5' }}>{d.issue}</span>
                                                <span style={{ fontSize: '9px', fontWeight: 700, padding: '2px 9px', borderRadius: '20px', background: d.status === 'open' ? 'rgba(239,68,68,0.12)' : d.status === 'resolved' ? 'rgba(16,185,129,0.12)' : 'rgba(167,139,250,0.12)', color: d.status === 'open' ? '#ef4444' : d.status === 'resolved' ? '#10b981' : '#a78bfa', border: `1px solid ${d.status === 'open' ? 'rgba(239,68,68,0.25)' : d.status === 'resolved' ? 'rgba(16,185,129,0.25)' : 'rgba(167,139,250,0.25)'}` }}>
                                                    {d.status === 'open' ? 'باز' : d.status === 'resolved' ? 'حل‌شده' : 'ارجاع داده‌شده'}
                                                </span>
                                            </div>
                                            <div style={{ fontSize: '12px', color: 'rgba(240,250,245,0.45)' }}>{d.match} · {d.player} · {d.time}</div>
                                        </div>
                                        {d.status === 'open' && (
                                            <div style={{ display: 'flex', gap: '6px', flexShrink: 0 }}>
                                                <button style={{ display: 'flex', alignItems: 'center', gap: '5px', padding: '8px 14px', background: 'rgba(16,185,129,0.1)', border: '1px solid rgba(16,185,129,0.25)', borderRadius: '10px', color: '#10b981', fontSize: '12px', fontWeight: 700, cursor: 'pointer', fontFamily: 'inherit' }}>
                                    ...
                                    <p style={{ fontSize: '13px', color: 'rgba(240,250,245,0.5)', margin: 0, lineHeight: 1.6, padding: '12px 14px', background: 'rgba(255,255,255,0.02)', border: '1px solid rgba(255,255,255,0.04)', borderRadius: '12px' }}>
                                        {d.description}
                                    </p>
                                </div>
                            ))}
